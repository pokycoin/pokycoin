...
*Setup instructions for a Gitian build of Pokycoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pokycoin
...
- [Building Pokycoin](#building-pokycoin)
...
Clone the git repositories for pokycoin and Gitian and then checkout the pokycoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/pokycoin-project/pokycoin.git
cd pokycoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pokycoin repository
...
To build Pokycoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the pokycoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pokycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/pokycoin-project/pokycoin
...
For example:
```bash
URL=https://github.com/user/pokycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pokycoin=${COMMIT} --url pokycoin=${URL} ../pokycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pokycoin=${COMMIT} --url pokycoin=${URL} ../pokycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pokycoin=${COMMIT} --url pokycoin=${URL} ../pokycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pokycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pokycoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pokycoin-build.assert
```
...
[pokycoin/gitian.sigs](https://github.com/pokycoin-project/gitian.sigs.POKY/) repository, or if that's not possible create a pull
request. 
